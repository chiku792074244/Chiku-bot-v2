...
module.exports.run = async function ({ api, event, Users }) {
  ...
  if (event.messageReply?.senderID) {
    ...
    if (!members.length)
      return api.sendMessage(
        "âŒ Group me koi aur member nahi hai.",
        event.threadID,
        event.messageID
      );
  ...
  if (!id2)
    return api.sendMessage("âŒ Pair nahi mila.", event.threadID, event.messageID);
  ...
  ctx.fillStyle = "red";
  ctx.fillText("â¤", W / 2, 160);
  ctx.fillStyle = "#000000";
  ctx.fillText("YOU", W / 2 + 170, 160);

  // ===== BOTTOM TEXT =====
  ctx.font = "bold 160px Arial";
  ctx.fillText("HÃºm TÃºm", W / 2, H - 120);
